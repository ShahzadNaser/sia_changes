...
def execute(filters=None):
	columns, data = [
		"Sales Person:Link/Sales Person:100"
	], []
	...
	data_sinv = frappe.db.sql("""  SELECT st.`sales_person`, si.`posting_date`, sii.`net_amount` AS `net_total` 
	...
	for i in data_sinv:
		...
		if i["sales_person"] in result:

			for k in result[i["sales_person"]]:
				start_date_update = k
				if filters.get("range") == 'Weekly':
				...
				if filters.get("range") in ['Monthly', 'Quarterly',"Weekly"] :
					if i["posting_date"] >= start_date_update and i["posting_date"] <= k : 
						result[i["sales_person"]][k] = result[i["sales_person"]][k] + i["net_total"]
			...
			if filters.get("range") == "Half-Yearly":
				...
				if str(i["posting_date"]) >= start_date and str(i["posting_date"]) <= end_date:
					result[i["sales_person"]][label] = result[i["sales_person"]][label]  +  i["net_total"]
		else:
			result[i["sales_person"]] = {}
			for j in periodic_daterange:
				result[i["sales_person"]].update({j : 0})
			...
			for k in result[i["sales_person"]]:
				...
				if filters.get("range") in ['Monthly', 'Quarterly',"Weekly"] :
					if i["posting_date"] >= start_date_new and i["posting_date"] <= k : 
						result[i["sales_person"]][k] = i["net_total"]
				elif filters.get("range") in ["Daily"]:
					if i["posting_date"] == k:
						result[i["sales_person"]][k] =  i["net_total"]
				elif filters.get("range") == "Yearly":
			...
			if filters.get("range") == "Half-Yearly":
				...
				if str(i["posting_date"]) >= start_date and str(i["posting_date"]) <= end_date:
					result[i["sales_person"]][label] =  i["net_total"]



	for i in data_sinv:
		result[i["sales_person"]].update({"net_total" : 0 })
	    

	for i in data_sinv:
		total = 0
		for j in result[i["sales_person"]]:
			if j != "net_total" :
				total += result[i["sales_person"]][j] 
		result[i["sales_person"]]["net_total"] = total
