...
def execute(filters=None):
	columns, data = [
		"Item Subgroup:Link/Item Subgroup:100"
	], []
	...
	if filters.get("item_subgroup"):
		item_tab = """ """
		item_cond = """ AND  i.`item_subgroup` = "{}"  """.format(filters.get("item_subgroup"))
	...
	data_sinv = frappe.db.sql("""  SELECT i.`item_subgroup`, si.`posting_date`, sii.`net_amount` AS `net_total` 
	...
	for i in data_sinv:
		...
		if i["item_subgroup"] in result:

			for k in result[i["item_subgroup"]]:
				start_date_update = k
				if filters.get("range") == 'Weekly':
				...
				if filters.get("range") in ['Monthly', 'Quarterly',"Weekly"] :
					if i["posting_date"] >= start_date_update and i["posting_date"] <= k : 
						result[i["item_subgroup"]][k] = result[i["item_subgroup"]][k] + i["net_total"]
			...
			if filters.get("range") == "Half-Yearly":
				...
				if str(i["posting_date"]) >= start_date and str(i["posting_date"]) <= end_date:
					result[i["item_subgroup"]][label] = result[i["item_subgroup"]][label]  +  i["net_total"]
		else:
			result[i["item_subgroup"]] = {}
			for j in periodic_daterange:
				result[i["item_subgroup"]].update({j : 0})
			...
			for k in result[i["item_subgroup"]]:
				...
				if filters.get("range") in ['Monthly', 'Quarterly',"Weekly"] :
					if i["posting_date"] >= start_date_new and i["posting_date"] <= k : 
						result[i["item_subgroup"]][k] = i["net_total"]
				elif filters.get("range") in ["Daily"]:
					if i["posting_date"] == k:
						result[i["item_subgroup"]][k] =  i["net_total"]
				elif filters.get("range") == "Yearly":
			...
			if filters.get("range") == "Half-Yearly":
				...
				if str(i["posting_date"]) >= start_date and str(i["posting_date"]) <= end_date:
					result[i["item_subgroup"]][label] =  i["net_total"]



	for i in data_sinv:
		result[i["item_subgroup"]].update({"net_total" : 0 })
	    

	for i in data_sinv:
		total = 0
		for j in result[i["item_subgroup"]]:
			if j != "net_total" :
				total += result[i["item_subgroup"]][j] 
		result[i["item_subgroup"]]["net_total"] = total
