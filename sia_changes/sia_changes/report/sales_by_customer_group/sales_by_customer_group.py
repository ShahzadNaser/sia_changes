...
def execute(filters=None):
	...
	data_sinv = frappe.db.sql("""  SELECT si.`customer_group`, si.`posting_date`, sii.`net_amount` AS `net_total` 
	...
	for i in data_sinv:
		...
		if i["customer_group"] in result:

			for k in result[i["customer_group"]]:
				start_date_update = k
				if filters.get("range") == 'Weekly':
				...
				if filters.get("range") in ['Monthly', 'Quarterly',"Weekly"] :
					if i["posting_date"] >= start_date_update and i["posting_date"] <= k : 
						result[i["customer_group"]][k] = result[i["customer_group"]][k] + i["net_total"]
			...
			if filters.get("range") == "Half-Yearly":
				...
				if str(i["posting_date"]) >= start_date and str(i["posting_date"]) <= end_date:
					result[i["customer_group"]][label] = result[i["customer_group"]][label]  +  i["net_total"]
		else:
			result[i["customer_group"]] = {}
			for j in periodic_daterange:
				result[i["customer_group"]].update({j : 0})
			...
			for k in result[i["customer_group"]]:
				...
				if filters.get("range") in ['Monthly', 'Quarterly',"Weekly"] :
					if i["posting_date"] >= start_date_new and i["posting_date"] <= k : 
						result[i["customer_group"]][k] = i["net_total"]
				elif filters.get("range") in ["Daily"]:
					if i["posting_date"] == k:
						result[i["customer_group"]][k] =  i["net_total"]
				elif filters.get("range") == "Yearly":
			...
			if filters.get("range") == "Half-Yearly":
				...
				if str(i["posting_date"]) >= start_date and str(i["posting_date"]) <= end_date:
					result[i["customer_group"]][label] =  i["net_total"]



	for i in data_sinv:
		result[i["customer_group"]].update({"net_total" : 0 })
	    

	for i in data_sinv:
		total = 0
		for j in result[i["customer_group"]]:
			if j != "net_total" :
				total += result[i["customer_group"]][j] 
		result[i["customer_group"]]["net_total"] = total
