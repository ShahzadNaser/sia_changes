...
def execute(filters=None):
	...
	data_sinv = frappe.db.sql("""  SELECT sii.`warehouse`, si.`posting_date`, sii.`net_amount` AS `net_total` 
	...
	for i in data_sinv:
		...
		if i["warehouse"] in result:

			for k in result[i["warehouse"]]:
				start_date_update = k
				if filters.get("range") == 'Weekly':
				...
				if filters.get("range") in ['Monthly', 'Quarterly',"Weekly"] :
					if i["posting_date"] >= start_date_update and i["posting_date"] <= k : 
						result[i["warehouse"]][k] = result[i["warehouse"]][k] + i["net_total"]
			...
			if filters.get("range") == "Half-Yearly":
				...
				if str(i["posting_date"]) >= start_date and str(i["posting_date"]) <= end_date:
					result[i["warehouse"]][label] = result[i["warehouse"]][label]  +  i["net_total"]
		else:
			result[i["warehouse"]] = {}
			for j in periodic_daterange:
				result[i["warehouse"]].update({j : 0})
			...
			for k in result[i["warehouse"]]:
				...
				if filters.get("range") in ['Monthly', 'Quarterly',"Weekly"] :
					if i["posting_date"] >= start_date_new and i["posting_date"] <= k : 
						result[i["warehouse"]][k] = i["net_total"]
				elif filters.get("range") in ["Daily"]:
					if i["posting_date"] == k:
						result[i["warehouse"]][k] =  i["net_total"]
				elif filters.get("range") == "Yearly":
			...
			if filters.get("range") == "Half-Yearly":
				...
				if str(i["posting_date"]) >= start_date and str(i["posting_date"]) <= end_date:
					result[i["warehouse"]][label] =  i["net_total"]



	for i in data_sinv:
		result[i["warehouse"]].update({"net_total" : 0 })
	    

	for i in data_sinv:
		total = 0
		for j in result[i["warehouse"]]:
			if j != "net_total" :
				total += result[i["warehouse"]][j] 
		result[i["warehouse"]]["net_total"] = total
...
# 	columns = ["Warehouse:Data:100","Net Amount:Currency:100"]
...
# 		item_code_clause = """ AND sinvi.`item_code` = "{}" """.format(filters.get("item_code"))
...
# 		GROUP BY sinvi.`warehouse`
# 	""".format(date_clause, customer_clause, customer_group_clause, item_group_clause, item_subgroup_clause, warehouse_clause, sub_customer_clause, item_code_clause, territory_clause, sales_person_tab, sales_person_cond))
